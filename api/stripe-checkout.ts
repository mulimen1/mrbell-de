...
//  - "setup_only"    → 99€ einmalig (Pfad "Geführtes Setup")
//
// Mindestlaufzeiten:
//  - Pioneer 6: 6 Monate
//  - Pioneer 3: 3 Monate
...
//
// onboardingId: wird vom Frontend nach /api/save-onboarding mitgeschickt.
// Sie verbindet die Stripe-Session mit dem Pre-Save im Master-Sheet.
//
// USt-Disclaimer (§19 UStG): wird auf JEDER Rechnung als Footer angehängt
// — sowohl bei setup_only als auch bei allen Subscriptions (Erst- + Folgezahlungen).
//
...
// Wird auf JEDER Stripe-Rechnung als Footer angehängt.
// ═══════════════════════════════════════════════════════════
...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    // Stripe-Metadata: nur die ESSENTIAL Felder.
    // Alle anderen Onboarding-Daten liegen bereits im Master-Sheet (via /api/save-onboarding)
    // und werden über die onboardingId verbunden.
    ...
    if (plan === "setup_only") {
      const session = await stripe.checkout.sessions.create({
        ...
        custom_text: {
          ...
          submit: {
            message:
              "Hinweis: Als Kleinunternehmer nach § 19 UStG wird keine Umsatzsteuer ausgewiesen.",
          },
        },
      });
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      custom_text: {
        ...
        submit: {
          message:
            "Hinweis: Als Kleinunternehmer nach § 19 UStG wird keine Umsatzsteuer ausgewiesen. " +
            "Nach 5 Tagen Trial startet automatisch die kostenpflichtige Mitgliedschaft.",
        },
      },
    });
